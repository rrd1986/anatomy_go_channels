...
func sqrWorkerWithWaitGroup(wg *sync.WaitGroup, tasks <-chan int, results chan<- int, instance int) {
	for num := range tasks {
...
func Workers_with_waitgroup() {
	...
	for i := 0; i < 3; i++ {
		wg.Add(1)
		go sqrWorkerWithWaitGroup(&wg, tasks, results, i)
	}
